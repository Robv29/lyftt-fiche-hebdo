...
/**
 * L'écran ne montrait qu'une seule journée : une date sans contenu affichait
 * « Rien à publier » alors que la semaine était chargée. On couvre désormais
 * deux semaines à partir de la date choisie, regroupées par jour.
 */
const HORIZON_DAYS = 14;
function horizonDate(from:string):string { return format(addDays(new Date(`${from}T12:00:00`), HORIZON_DAYS), "yyyy-MM-dd"); }
...
export default async function PublicationsPage({ searchParams }:{ searchParams:Promise<{date?:string}> }) {
  ...
  const { data:rows }=await supabase.from("weekly_sheet_items").select(`id, scheduled_date, scheduled_time, format, networks, caption, hashtags, approval_status, published_at, media_downloaded_at, content_copied_at, media_external_url, media_pending_note, media_assets:media_asset_id ( kind, storage_path, file_name, preview_path, purged_at, preview_purged_at ), weekly_sheets!inner ( clients ( name ) )`).gte("scheduled_date",date).lte("scheduled_date",horizonDate(date)).eq("is_cancelled",false).order("scheduled_date",{ascending:true}).order("scheduled_time",{ascending:true});
