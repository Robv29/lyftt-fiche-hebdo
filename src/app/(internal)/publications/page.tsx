...
export default async function PublicationsPage({ searchParams }:{ searchParams:Promise<{date?:string}> }) {
  ...
  const { data:rows }=await supabase.from("weekly_sheet_items").select(`id, scheduled_time, format, networks, caption, hashtags, approval_status, published_at, media_downloaded_at, content_copied_at, media_external_url, media_pending_note, media_assets:media_asset_id ( kind, storage_path, file_name ), weekly_sheets!inner ( clients ( name ) )`).eq("scheduled_date",date).eq("is_cancelled",false).order("scheduled_time",{ascending:true});
