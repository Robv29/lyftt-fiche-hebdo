...
export default async function PublicationsPage({ searchParams }:{ searchParams:Promise<{date?:string}> }) {
  ...
  const { data:rows }=await supabase.from("weekly_sheet_items").select(`id, scheduled_date, scheduled_time, format, networks, caption, hashtags, approval_status, published_at, media_downloaded_at, content_copied_at, media_external_url, media_pending_note, published_networks, media_assets:media_asset_id ( kind, storage_path, file_name, preview_path, purged_at, preview_purged_at ), weekly_sheet_item_media ( position, media_assets ( kind, storage_path, file_name, preview_path, purged_at, preview_purged_at ) ), weekly_sheets!inner ( clients ( name, notes ) )`).eq("scheduled_date",date).eq("is_cancelled",false).order("scheduled_date",{ascending:true}).order("scheduled_time",{ascending:true});
