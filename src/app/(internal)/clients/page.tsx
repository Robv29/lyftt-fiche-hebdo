...
import { cadenceMonthlyCostCents, parseShootingPlan } from "@/lib/domain/budget";
...
export default async function ClientsPage() {
  ...
  return <div className="space-y-7">
    ...
    <ClientAdmin clients={await Promise.all((clients ?? []).map(async (c) => {
      ...
      let settings: { monthlyCadence?: { photo?:number; video?:number; story?:number; visual?:number }; shootingPlan?: unknown } = {};
      ...
      const monthlyCostCents = profile.role === "super_admin"
        ? cadenceMonthlyCostCents(cadence, parseShootingPlan(settings.shootingPlan))
        : null;
