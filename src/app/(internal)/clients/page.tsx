...
export default async function ClientsPage() {
  ...
  const [{ data: clients }, { data: managers }] = await Promise.all([
    supabase.from("clients").select("id, name, logo_url, notes, is_active, validation_deadline_weekday, validation_deadline_time, approval_policy, contract_end_date, pause_start_date, pause_end_date, client_contacts ( first_name, last_name, phone ), client_assignments ( role, profiles ( full_name ) )").order("name"),
