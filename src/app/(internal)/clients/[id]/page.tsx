...
import { ConvertToManagement } from "./ConvertToManagement";
...
export default async function ClientPage({ params }: { params: Promise<{ id: string }> }) {
   ...
   */
  // Le commercial consulte la fiche : aucun formulaire ne lui est présenté.
  const readOnly = profile?.role === "commercial";
  /*
   * Client en prestation ponctuelle : ni cadence, ni fiches, ni échéance de
   * validation. L'éditeur de gestion n'est pas proposé — il exige les cinq
   * hashtags, les réseaux, le groupe WhatsApp : le client serait impossible à
   * enregistrer. On propose à la place de le passer en gestion.
   */
  const oneShot = parseClientKind(client.client_kind) === "ponctuel";
  const prestationPonctuelle = typeof settings.prestationPonctuelle === "string"
    ? settings.prestationPonctuelle
    : "";
      ...
      <div className="space-y-6">
        {oneShot ? (
          <section className="card p-4 sm:p-6">
            <p className="eyebrow">Prestation ponctuelle</p>
            <h2 className="mt-1 text-lg font-semibold">{prestationPonctuelle || "Prestation à préciser"}</h2>
            <p className="mt-2 text-sm text-ink-soft">
              Client sans gestion des réseaux : aucune fiche hebdomadaire, aucun mois facturé.
              La prestation vendue s&apos;inscrit dans son budget.
            </p>
          </section>
        ) : (
