...
export default async function ClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-7">
    <header><Link href="/clients" className="mb-5 inline-flex min-h-11 items-center text-sm text-ink-soft hover:text-ink">← Clients</Link><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-4"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#e8effa] text-lg font-bold text-[#3e526f]">{client.name.slice(0,2).toUpperCase()}</span><div><h1 className="page-title">{client.name}</h1><p className="mt-1 text-sm text-ink-soft">{client.is_active ? "Collaboration active" : "Client en pause"} · {client.timezone}</p></div></div></div></header>
    <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
      <div className="space-y-6">
        <section className="card p-6"><div className="flex items-center justify-between"><div><p className="eyebrow">Cadence éditoriale</p><h2 className="mt-1 text-lg font-semibold">Prestations en cours</h2></div><span className="rounded-full bg-[#edf4ff] px-3 py-1 text-xs font-semibold text-[#1468ff]">Calculée sur 4 semaines</span></div>
        ...
        <section className="card overflow-hidden"><div className="flex items-center justify-between border-b px-6 py-5"><div><p className="eyebrow">Historique</p><h2 className="mt-1 font-semibold">Fiches hebdomadaires</h2></div><Link href="/fiches" className="text-xs font-semibold text-[#1468ff]">Tout voir</Link></div>{sheets.length ? <ul className="divide-y">{sheets.slice(0,6).map(s=><li key={s.id}><Link href={`/fiches/${s.id}`} className="flex items-center gap-4 px-6 py-4 hover:bg-canvas"><span className="grid h-10 w-10 place-items-center rounded-xl bg-canvas text-xs font-bold">S{s.iso_week}</span><div className="min-w-0 flex-1"><strong className="text-sm">Semaine {s.iso_week}</strong><p className="text-xs text-ink-faint">{s.weekly_sheet_items.length} contenu{s.weekly_sheet_items.length>1?"s":""}</p></div><span className="badge bg-canvas text-ink-soft">{sheetStatusLabel(s.status)}</span><Icon name="arrow" className="h-4 w-4 text-ink-faint"/></Link></li>)}</ul> : <p className="p-8 text-center text-sm text-ink-faint">Aucune fiche créée pour ce client.</p>}</section>
